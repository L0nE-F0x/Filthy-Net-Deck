"""Generate youtube-post.png for YouTube community posts (1280x720, 16:9)."""
...
def main() -> None:
    ...
    img = Image.alpha_composite(img, soft_orb((W, H), 230, 380, 460, ACID, 55))
    img = Image.alpha_composite(img, soft_orb((W, H), 1130, 560, 400, (212, 168, 75), 45))
    img = Image.alpha_composite(img, soft_orb((W, H), 980, 90, 280, (92, 107, 42), 35))
    ...
    tag_font = load_font(bold, 42)
    body_font = load_font(regular, 30)
    chip_font = load_font(bold, 22)
    small_font = load_font(regular, 24)
    ...
    # Live dot
    draw.ellipse([tx - 20, ty + 9, tx - 7, ty + 22], fill=(52, 211, 153, 255))

    draw.text((tx, ty + 44), "Filthy Net Deck", font=title_font, fill=FOAM)
    draw.text((tx, ty + 142), "Netdeck dirty. Climb clean.", font=tag_font, fill=ACID_BRIGHT)
    ...
    # Bottom bar
    draw.rectangle([0, H - 60, W, H], fill=(10, 11, 8, 245))
    draw.rectangle([0, H - 60, W, H - 58], fill=(*ACID, 200))
    draw.text((80, H - 43), "v1.1.1  ·  Windows + macOS", font=small_font, fill=MUTED)
    draw.text((tx, H - 43), "filthy-net-deck.netlify.app", font=small_font, fill=GOLD_LIGHT)
